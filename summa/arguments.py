...
import torch
from summa import fused_kernels

def rank_rearrange(rank, summa_dim, model_parallel_size):
    num_devices = torch.cuda.device_count()
    assert num_devices == 4, "for TACC, otherwise please not rank-rearrange"
    intra_model_rank = rank % model_parallel_size
    ddp_rank = rank // model_parallel_size
    block_number = intra_model_rank // 4
    blocks_per_row = summa_dim // 2
    block_row = block_number // blocks_per_row
    block_col = block_number % blocks_per_row
    intra_block_rank = intra_model_rank - block_number * 4
    intra_block_row = intra_block_rank // 2
    intra_block_col = intra_block_rank % 2

    new_intra_model_row = block_row * 2 + intra_block_row
    new_intra_model_col = block_col * 2 + intra_block_col
    new_intra_model_rank = \
        new_intra_model_row * summa_dim + new_intra_model_col
    new_rank = new_intra_model_rank + model_parallel_size * ddp_rank
    return new_rank
...
def parse_args(extra_args_provider=None, defaults={},
    ...
    args.rank_original = rank
    if args.rank_rearrange:
        args.rank = rank_rearrange(rank, args.summa_dim, args.model_parallel_size)
    else:
        args.rank = rank
    ...
    args.init_method = 'tcp://' + master_addr + ':' + args.master_port
    ...
    # Checks.
    assert args.checkpoint_num_layers == 1,\
        'only this allowed'
    assert args.model_parallel_size == args.summa_dim ** 2,\
        'model_parallel_size should equal to summa_dim square'
    assert args.batch_size % args.summa_dim == 0,\
        'batch_size should be divisible by summa_dim'
    assert args.hidden_size % args.summa_dim == 0,\
        'hidden size should be divisible by summa_dim'
    assert args.max_position_embeddings == args.seq_length,\
        'discrepancy!'
    assert args.num_attention_heads % args.summa_dim == 0,\
        'attention heads per partition!'
...
def _add_learning_rate_args(parser):
    ...
    group.add_argument('--lr-decay-iters', type=int, default=990000,
                       help='number of iterations to decay learning rate over,'
                       ' If None defaults to `--train-iters`')
    group.add_argument('--min-lr', type=float, default=0.00001,
...
def _add_checkpointing_args(parser):
    ...
    group.add_argument('--save-interval', type=int, default=10000,
...
def _add_mixed_precision_args(parser):
    ...
    group.add_argument('--fp16', default=False,
...
def _add_distributed_args(parser):
    ...
    group.add_argument('--master-port', type=str, default='6006',
                       help='method to initialize torch.distributed')
    group.add_argument('--rank-rearrange', action='store_true',
                       help='whether to rearrange rank')
    group.add_argument('--model-parallel-size', type=int, default=4,
                       help='Size of the model parallel.')
    group.add_argument('--summa-dim', type=int, default=2,
                       help='Size of summa matrix partition,'
                            'model_parallel_size = summa_dim ** 2')
...
def _add_data_args(parser):
    ...
    group.add_argument('--data-path', type=str, default='/work/07789/xuqifan/frontera/dataset/bert_data/my-bert_text_sentence',
                       help='Path to combined dataset to split.')
    group.add_argument('--split', type=str, default='949, 50, 1',
                       help='Comma-separated list of proportions for training,'
                       ' validation, and test split. For example the split '
                       '`90,5,5` will use 90% of data for training, 5% for '
                       'validation and 5% for test.')
    group.add_argument('--vocab-file', type=str, default='/work/07789/xuqifan/frontera/dataset/bert_data/vocab.txt',
