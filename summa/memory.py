...
class MemoryBuffer:
    ...
    def add(self, tensor):
        ...
        assert new_start <= self.numel, \
            'Not enough memory left in the buffer ({} > {}), name: {}'.format(
                tensor_numel, self.numel - self._start, self.name)
    ...
    def zeros(self, shape):
        """allocate zeros of shape"""
        tensor_numel = functools.reduce(operator.mul, shape)
        new_start = self._start + tensor_numel
        assert new_start <= self.numel, \
            'Not enough memory left in the buffer ({} > {}), name: {}'.format(
                tensor_numel, self.numel - self._start, self.name)
...
class SimpleBuffer():
    ...
    def zeros(self, shape):
        assert shape == self.tensor.shape
        self.tensor.zero_()
        return self.tensor

    def add(self, input):
        self.tensor.copy_(input)
        return self.tensor